...
import Simplex.Messaging.Agent.Store.SQLite (SQLiteStore, withTransaction, MigrationConfirmation (MCYesUp, MCConsole))
...
simplexChatCore cfg@ChatConfig {confirmMigrations, testView} opts@ChatOpts {coreOptions = CoreChatOpts {dbFilePrefix, dbKey, logAgent, yesToUpMigrations}, displayName} chat =
  case logAgent of
    Just level -> do
      setLogLevel level
  ...
  where
    initRun = do
      let confirmMigrations' = if confirmMigrations == MCConsole && yesToUpMigrations then MCYesUp else confirmMigrations
      createChatDatabase dbFilePrefix dbKey False confirmMigrations' >>= either exit run
    exit e = do
